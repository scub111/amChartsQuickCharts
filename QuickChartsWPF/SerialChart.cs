...
namespace AmCharts.Windows.QuickCharts
{
    ...
    public class SerialChart : Control
    {
        ...
        private void AssignGridParts()
        {
            ...
            Binding valueFormatBinding = new Binding("ValueFormatString");
            valueFormatBinding.Source = this;
            _valueAxis.SetBinding(ValueAxis.ValueFormatStringProperty, valueFormatBinding);
        ...
        {
            int index = GetValueIndexByCoordinateX(position.X);
            if (index > -1)
            {
                string tooltipContent;
                for (int i = 0; i < _graphs.Count; i++)
                {
                    if (string.IsNullOrEmpty(_graphs[i].Title))
                        tooltipContent = _categoryValues[index] + " | "
                        ...
                        tooltipContent = _graphs[i].Title + ": " + _categoryValues[index] + " | "
                            + (string.IsNullOrEmpty(ValueFormatString) ? _values[_graphs[i].ValueMemberPath][index].ToString() : _values[_graphs[i].ValueMemberPath][index].ToString(ValueFormatString));
        ...
        {
            int valueIndex = GetValueIndexByCoordinateX(position.X);
            if (valueIndex > -1)
            {
                ...
                foreach (SerialGraph graph in _graphs)
                {
                    ...
                    point = _locations[graph.ValueMemberPath][valueIndex];
                    posYs.Add(point.Y);
                    _indicators[graph].SetPosition(point);
                }
                int graphIndex = GetGraphIndexByCoordinateY(position.Y, posYs);
                DisplayBalloon(_indicators[_graphs[graphIndex]]);
            }
            //_balloon.Visibility = Visibility.Collapsed;
        }
        ...
        private void ProcessCategoryData()
        {
            ...
            if (this.DataSource != null && !string.IsNullOrEmpty(CategoryValueMemberPath))
            {
                ...
                foreach (object dataItem in this.DataSource)
                {
                    if (!string.IsNullOrEmpty(CategoryFormatString) && eval.Eval(dataItem) is IFormattable)
                        _categoryValues.Add(((IFormattable)eval.Eval(dataItem)).ToString(CategoryFormatString, CultureInfo.CurrentCulture));
                    else
        ...
        private int GetValueIndexByCoordinateX(double x)
        {
            int index = -1;
        ...
        private int GetGraphIndexByCoordinateY(double y, List<double> posYs)
        {
            int index = 0;
            double min = Math.Abs(y - posYs[0]);

            for (int i = 1; i < posYs.Count; i++)
                if (min > Math.Abs(y - posYs[i]))
                {
                    min = Math.Abs(y - posYs[i]);
                    index = i;
                }
        ...
        /// <summary>
        /// Gets or sets the format string used to format values on Y-axes and in tooltips.
        ...
        /// <summary>
        /// Gets or sets the format string used to format values on X-axes and in tooltips.
        /// This is a depenency property.
        /// </summary>
        public string CategoryFormatString
        {
            get { return (string)GetValue(SerialChart.CategoryFormatStringProperty); }
            set { SetValue(SerialChart.CategoryFormatStringProperty, value); }
        }
    }
}
